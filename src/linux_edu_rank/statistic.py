...
def main():
    ...
    for commit in tqdm(commits):
        ...
        if not utils.is_university_domain(domain, university_list):
            continue
    ...
    result = map(
        lambda x: {
            ...
            "university": utils.get_university(x[0], university_list),
        },
    ...
    for item in result:
        ...
        if item["university"] is None:
            authors = utils.result_authors_transform(result_authors, item)
        ...
        if item["domain"] not in result_tmp[name]["domains"]:
            ...
            result_tmp[name]["authors"].extend(utils.result_authors_transform(result_authors, item))
    ...
    for item in result:
        ...
        utils.generate_html(item["id"], "Patches contributed by " + item["name"], patches)
